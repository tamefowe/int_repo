...
class DataProcessor(object):
    ...
    def _is_trade_valid(trade):
        try:
            ...
            print('ERROR: Missing side, fill size/price, exchange!', file=sys.stderr)
            return False

        if event_type != 'TRADE':
            print('ERROR: Invalid event type {}'.format(event_type), file=sys.stderr)
            return False
        try:
            _ = int(fill_size)
            _ = float(fill_price)
            ...
            print('ERROR: Invalid data for fill size {} or fill price {} or side {}'.format(fill_size, fill_price, side),
                  file=sys.stderr)
            return False
        return True
